...
// HasStudytimes applies the HasEdge predicate on the "studytimes" edge.
func HasStudytimes() predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasStudytimesWith applies the HasEdge predicate on the "studytimes" edge with a given conditions (other predicates).
func HasStudytimesWith(preds ...predicate.StudyTime) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := newStudytimesStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
